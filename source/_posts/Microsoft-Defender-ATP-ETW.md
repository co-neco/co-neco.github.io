...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-02-2-Microsoft-Defender-ATP-kernel-initiating-code-injection-alert.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-03-dumpbin-utility-used-to-detect-user-APC-injection-primitives.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-04-User-APC-injection-code.png)

这个APC的NormalRoutine是一段shellcode，其调用*CreateProcessW*，用刚提到的命令行创建了一个进程。这意味着通过APC向*services.exe*实施的代码注入的目的是生成一个子进程。

![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-05-User-shellcode-performing-process-creation.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-06-2-Breakpoint-hit-on-the-call-to-memcpy_s-copying-shellcode-parameters.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-07-MateBookService-exe-startup-code-path.png)

分析到这里，最后需要确定的是退出的进程，同时也是引发代码注入的这个进程是否就是*MateBookService.exe*。

![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-08-Validating-terminated-process-identity.png)

观察用于决定是否注入代码到*services.exe*的代码片段，该代码片段用了一个全局列表，这个全局列表包含了需要监控的进程名。通过在这个循环里设置一个断点，我们发现这个全局列表只有一个元素，是*MateBookService.exe*。

![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-09-Breakpoint-hit-during-process-name-comparison-against-global-list.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-10-HwOs2Ec10x64.sys-control-device-creation-with-IoCreateDevice.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-11-IRP_MJ_CREATE-dispatch-routine.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-12-Procmon-utility-results-showing-POC-process-start-exit-IL.png)

因为被“照顾”的进程退出时，它们会被看门狗盲目地重启，所以攻击者控制的程序也会被services.exe重启，这意味着攻击者控制的进程的权限是LocalSystem用户和权限一样，也就是得到了提权。

![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-13-Procexp-utility-process-tree-view-showing-LPE_POC-running-as-LocalSystem.png)
...
![](https://gitee.com/co-neco/pic_bed/raw/master/translation/figure-14-2-Microsoft-Defender-ATP-alerting-on-the-privilege-escalation-POC-code.png)
