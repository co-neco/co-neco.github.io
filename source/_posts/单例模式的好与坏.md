...
单例模式可以带来很多好处，因为我们不需要频繁地创建单例类的实例，在需要单例类的实例时，调用getInstance获取即可。

然后，单例模式也会带来一些问题，比如这个[讨论](https://stackoverflow.com/questions/137975/what-are-drawbacks-or-disadvantages-of-singleton-pattern)和[这篇文章](http://misko.hevery.com/2008/08/25/root-cause-of-singletons/)。
...
如果SingletonA和SingletonB的Instance都是第一次被调用，那么SingletonA先被构造，之后是SingletonB，在进程退出时，SingletonB会先被析构，如果此时进程的某个线程正要使用SingletonB的实例，那么由于SingletonB的实例被析构了，所以能可能出现访问异常，导致崩溃。
...
这个[链接](https://stackoverflow.com/a/138012)描述的很详细，可以仔细研读。

## 总结

单例模式这个设计模式确实存在它的问题，但单例模式的概念是好的，没有问题的。之所以被大家诟病的原因，其实是部分开发者对于单例模式的不当使用。

另外，单例模式的使用中，需要注意单例类之间的初始化顺序，当然也要尽量避免单例类之间的依赖（不过这往往不太好避免）。

单例模式不易使用的场景是包含重要状态的一些类，这些类最好使用依赖传递。因为使用单例模式的话，会隐藏依赖关系，这样对于状态的改变就难以跟踪和分析。
