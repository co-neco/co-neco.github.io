...
如果SingletonA和SingletonB的Instance都是第一次被调用，那么SingletonA先被构造，之后是SingletonB，在进程退出时，SingletonB会先被析构，如果此时进程的某个线程正要使用SingletonB的实例，那么由于SingletonB的实例被析构了，所以可能出现访问异常，导致崩溃。
